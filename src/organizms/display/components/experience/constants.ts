import { JobProps } from "./interfaces";

export const EXPERIENCE: JobProps[] = [
  {
    company: "SberTech",
    companyLink: "https://sber-tech.com/",
    job: "React Frontend Developer",
    period: {
      from: "August 2021",
      to: "",
    },
    description:
      "Educational platforms department, UIKit Team. <br /> Design system development and its implementation in frontend apps <br /> <a href='https://uikit.pcbltools.ru/v4.1/' target='_blank'>Watch design system storybook</a> <br /> <a href='https://sberclass.ru/' target='_blank'>Watch platform using design system</a>",
    pic: "https://firebasestorage.googleapis.com/v0/b/arturovapp.appspot.com/o/images%2Fjobs%2Fsber.png?alt=media&token=26a972a0-adda-494f-b9eb-ca04b43c4749",
  },
  {
    company: "Obmen.Market",
    companyLink: "https://obmen.market/",
    job: "React Frontend Developer",
    period: {
      from: "May 2021",
      to: "July 2021",
    },
    description:
      "Private development. <br /> See Works section for more details",
    pic: "https://firebasestorage.googleapis.com/v0/b/arturovapp.appspot.com/o/images%2Fjobs%2Fobmen.png?alt=media&token=816a4dcb-f295-42d1-b2a0-59176c93dcd3",
  },
  {
    company: "IntraVision",
    companyLink: "https://intravision.ru/",
    job: "React Frontend Developer",
    period: {
      from: "March 2021",
      to: "April 2021",
    },
    description: "UI development",
    pic: "https://firebasestorage.googleapis.com/v0/b/arturovapp.appspot.com/o/images%2Fjobs%2Fintra.png?alt=media&token=9d9f7fee-1d40-4848-bd8b-5f01a0d61de1",
  },
  {
    company: "IT Education",
    job: "Frontend learner",
    period: {
      from: "July 2020",
      to: "Feb 2021",
    },
    description: "",
  },
  {
    company: "...rest",
    job: "non IT",
    period: {
      from: "long time ago",
      to: "Jun 2020",
    },
    description: `
      <small>Dec 2017 - Jun 2020</small> - PRO Photographer in <a href='https://swandorhotels.com/en/hotels/swandor-cam-rahn'><b>PhotoTeam Swandor Cam Ranh</b></a> <br />
      <small>Jul 2015 - Nov 2017</small> - Brand manager in <a href='https://www.muztorg.ru/'><b>MuzTorg</b></a> <br />
      <small>Nov 2011 - Nov 2014</small> - Procurement manager in <b>Enter</b> (no longer exist) <br />
      <small>Feb 2010 - Sep 2011</small> - Production Planning Manager in <a href='http://www.progressfood.ru/'><b>Progress</b></a> <br /> 
      etc...
    `,
  },
];
